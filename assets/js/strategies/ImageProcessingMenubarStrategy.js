...
// --------------------------------------
    ...
    ImageProcessingMenubarStrategy.prototype.listening = function(state){
      ...
      var updateValueFromGyroscope = function(acceleration){
          ...
          // remember sign
          var mult = acceleration.x < 0 ? -1 : 1
          var value = Math.abs(Math.floor(acceleration.x))
          // grow values over 2 expotentially
          value = Math.abs(value) <= 2 ? value : value*value
          // add delta
          this.currentValue -= value*mult
          ...
          if(oldValue != this.currentValue){
            ...
            // action
            $( "#"+this.name+"-slider-output" ).html(this.currentValue )
            $( "#"+this.name+"-slider").slider( "value",this.currentValue);
      ...
      if(state){
        console.log("START listening for: "+this.name)
        callMe()
      }else{
        console.log("STOP listening for: "+this.name)
      }

    }

    ImageProcessingMenubarStrategy.prototype.onError = function(error){
